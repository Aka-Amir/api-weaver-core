...
export class Schema extends ApiWeaverClass {
  ...
  private async generateInterface() {
    ...
    if (this.schema.description) {
      tags.push(`* @description ${this.schema.description}`);
    }

    if (this.schema.example) {
      tags.push(
        `* @example \`${typeof this.schema.example === "object" ? JSON.stringify(this.schema.example, undefined, 2) : this.schema.example}\``
      );
    }
  ...
  private async processAsObject(schema: ObjectSchemaType) {
    ...
    for (const [key, value] of Object.entries(schema.properties || {})) {
      ...
      if (value.example) {
        tags.push(`* @example \`${value.example}\``);
      }
      ...
      if (schema.required?.includes(key))
        properties.push(`${prefix}\t${key}: ${type};`);
      else properties.push(`${prefix}\t${key}?: ${type};`);
    }
    return properties;
  }
  ...
  private async parseSchemaType(property: SchemaType) {
    if ("$ref" in property) {
      ...
      return this.parseRefType((property.allOf as { $ref: string })[0]);
    }
